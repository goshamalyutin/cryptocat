...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-white/[0.07] pb-[34px] pt-[60px]">
      <div className="wrap">
        <div className="mb-[40px] grid grid-cols-2 gap-[30px] md:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            ...
            <div key={col.id}>
              ...
              {col.links.map((l) => (
                <Link
                  key={l.id}
                  href={l.href}
                  className="mb-2.5 block text-[14.5px] text-[#bdb2ba] transition-colors duration-200 hover:text-orange"
                >
                  {items[l.id as keyof typeof items]}
                </Link>
              ))}
            </div>
          ))}
